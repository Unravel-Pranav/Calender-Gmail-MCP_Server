...
# Build scopes list: prefer GOOGLE_SCOPES (comma-separated). Otherwise combine CALENDAR_SCOPES and GMAIL_SCOPES (comma-separated each)
_google_scopes_env = os.getenv('GOOGLE_SCOPES')
_calendar_scopes_env = os.getenv('CALENDAR_SCOPES', 'https://www.googleapis.com/auth/calendar')
_gmail_scopes_env = os.getenv('GMAIL_SCOPES', '')

if _google_scopes_env:
    SCOPES = [s.strip() for s in _google_scopes_env.split(',') if s.strip()]
else:
    scopes_combined = []
    if _calendar_scopes_env:
        scopes_combined.extend([s.strip() for s in _calendar_scopes_env.split(',') if s.strip()])
    if _gmail_scopes_env:
        scopes_combined.extend([s.strip() for s in _gmail_scopes_env.split(',') if s.strip()])
    SCOPES = scopes_combined if scopes_combined else ['https://www.googleapis.com/auth/calendar']
